...
### 1. データ指向設計の高速ランタイム（Nezia コアの強み）

Nezia 本体（[`nezia-core`](https://github.com/NeziaEngine/nezia-core)）は
Rust 製のサウンドミドルウェア。サウンドオブジェクトをコンポーネント単位で
密配列に格納する **スパースセット型 ECS** により、数百〜数千の同時発音を
キャッシュフレンドリーに一括処理する。オーサリング向けの不変な論理 ID と
ランタイム向けの高速な物理 ID を分けた **二層 ID 設計** で、
エディタ参照を壊さずにランタイム性能を出す。Unity 側へは C ABI（FFI）で
インプロセス連携するため、プロセス間通信のオーバーヘッドは無い。
